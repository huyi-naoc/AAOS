...
def create_template_fits_file(pathname):
    ...
    phdu.header['TELESCOP'] = ('AIRBT', 'telescope used to acquire data')
    ...
    phdu.header['INSTRUME'] = ('AIRBT-J', 'identifier for instrument used to acquire data')
    phdu.header['FILTER'] = ('J', 'filter in use')

    phdu.header['DETNAME'] = ('Leading IngaAs', 'detector in use')
    phdu.header['DETSIZE'] = ('640x512', 'detector format')
    phdu.header['COOLTEMP'] = (0.0, 'setting cooling temperature (Celsius degree)')
    phdu.header['NCHIP'] = (1, 'number of mosaic chips')
    phdu.header['X_OFFSET'] = (0, 'X offset of ROI')
    phdu.header['Y_OFFSET'] = (0, 'Y offset of ROI')
    phdu.header['IMGWIDTH'] = (640, 'width of ROI')
    phdu.header['IMGHEIGH'] = (512, 'height of ROI')
    phdu.header['X_BIN'] = (1, 'X binning')
    phdu.header['Y_BIN'] = (1, 'Y binning')
    phdu.header['GAIN'] = (5.00, 'requested gain (electrons/ADU)')
    phdu.header['GAINMODE'] = (False, 'whether auto gain enable')
    phdu.header['TRIGMODE'] = ('default', 'trigger mode')

    phdu.header['IMGENH'] = (0, 'image enhance mode, 0:off -- 1:hist -- 2:linear -- 3:mixed')
    phdu.header['CTRENH'] = (10, 'image contrast enhance coefficiency')
    phdu.header['DTENH'] = (0, 'image detail enhance coefficiency, 0 means disbaled')
    phdu.header['NFILTER'] = (0, 'noise filter coefficiency, 0 means disbaled')
    phdu.header['GMODE'] = (0, 'gamma correction mode, 0:off -- 1:manual -- 2:auto')
    phdu.header['NONEUNI'] = (False, 'whether none uniformaty correction enabled')
    phdu.header['BADPOINT'] = (False, 'whether bad point correction enabled')
    phdu.header['ENCMODE'] = (False, 'color encoding mode, T:PAL -- F:NTSC')
    phdu.header['FPATEMP'] = (0.00, 'actual FPA temperature of detector')
    phdu.header['SHELTEMP'] = (0.00, 'shell temperature of detector')
    phdu.header['SIGTEMP'] = (0.00, 'shell temperature of detector')
    phdu.header['TEMPMODE'] = (0, 'cooling mode, 0:off -- 1:manual -- 2:auto')
    ...
    phdu.header['SITENAME'] = ('Dome A', 'name of site')
    phdu.header['TEL_ID'] = (1, 'site\'s ID')
    phdu.header['SITE_LAT'] = ('-802503', 'latitude of the site (+/-DDMMSS.SS)')
    phdu.header['SITE_LON'] = ('+770532', 'logitude of the site (+/-DDDMMSS.SS)')
    phdu.header['SITE_ALT'] = (4090., 'altitude of the site (m)')
